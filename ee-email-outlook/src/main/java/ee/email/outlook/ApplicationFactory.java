...
public class ApplicationFactory implements EmailParsingFactory<Email> {
	...
	public synchronized Application getApplication(boolean initImmediate) {
		...
		if (this.application == null) {
			...
			OleClientSite site0 = new OleClientSite(frm, SWT.NONE,
					"OVCtl.OVCtl");
	...
	public EmailParsingController<Email> getEmailParsingController() {
		...
		if (this.emailParsingController == null) {
			...
			this.emailParsingController = new OutlookParsingController(
					getApplication(false), folderFilterForRecursion,
					folderFilter);
			;
		}
	...
	protected RegExpFolderFilter createFolderFilter() {
		...
		RegExpFolderFilter folderFilter = new RegExpFolderFilter(
				getRequiredSystemProperty(SYS__REG_EXP_FOR_FOLDER), true);
		return folderFilter;
	}
